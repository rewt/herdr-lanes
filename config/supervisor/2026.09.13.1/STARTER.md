...
working branch and current user goal before any lifecycle action. For a Codex
session, check its startup `directory:` against the approved supervisor
worktree. When using the shared remote daemon, require `-C` for every newly
started Codex engineer or reviewer and verify its startup directory before
prompting. Read the declared authoritative goal sources and latest durable
transition record. If
`TODAY.md` predates the current local day or a
...
Respect the active queue owner recorded in `PROJECT.md` and verified against
current evidence. A daily model default is a future launch choice, not an
instruction to replace an active specialist supervisor. A model override must
be explicit in the new session's launch arguments and recorded in `TODAY.md`.
...
project's own policy. Read CLOSEOUT.md from this same protocol pack. When the
operator has delegated promotion and close, complete accepted engineering lanes
through that sequence after the required checks, without asking again for
already-granted approval. Finish by verifying Git and Herdr cleanup and recording
any remaining cleanup blocker. Never push unless this session's operator explicitly
requests it.
...
handoff. Start a fresh supervisor session on a new day or major phase. If this
conversation grows beyond a useful context, compact or hand
