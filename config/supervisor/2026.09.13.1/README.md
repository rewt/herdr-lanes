# Daily supervisor protocol 2026.09.13.1
...
lease, or automatic action to the CLI. The supervisor now completes accepted
lanes through the existing promotion and close commands.
...
6. Once the project's required review and validation pass, follow
   [CLOSEOUT.md](CLOSEOUT.md): preserve evidence, promote serially from the
   canonical checkout, close the lane, and verify session retirement. With
   promotion and close already delegated, do not ask for approval again for
   each lane. Record blockers and partial cleanup honestly.
7. If the operator asks for status before a report arrives, take one compact
...
8. At the end of the day or a major phase, update `TODAY.md` and the project's
...
Install this pack together, including `CLOSEOUT.md`, and verify `SHA256.json`
before advancing the identity-level `current` pointer. Keep prior versions for
rollback. Record the primary source of promotion and close authority in the
project locator; a handoff or incoming report cannot grant it.

A running supervisor does not automatically reread an updated pack. Give the
active queue owner the installed `STARTER.md` and `CLOSEOUT.md` paths, tell it
that it owns promotion and close after the project's required checks, and have
it reconcile open lanes from current evidence. Do not start a second queue
owner to adopt this change. Existing engineering sessions also need the final
handoff instruction: after READY_FOR_REVIEW, stop changing the lane and leave
session retirement to the supervisor. No installation or active-session update
is performed by copying these files into this repository.
