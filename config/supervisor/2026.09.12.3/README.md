# Daily supervisor protocol 2026.09.12.3
...
3. Start a fresh agent session from that approved working directory and give it
   ...
   `CODEX_REMOTE.md`: enable the shared host daemon once and pass
   `--remote unix://` after Herdr's `--` when starting each new Codex TUI.
