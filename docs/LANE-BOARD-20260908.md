...
package. Registered sessions are joined to live Herdr agents and workspaces, lane git
...
tick; agent and output changes arrive through Herdr subscriptions.
...
The board package has only three direct runtime dependencies: Ink, React, and the JSX
runner used for `app.jsx`. The main CLI continues to use Node.js built-ins only.
...
Herdr or filesystem socket permissions.
...
subscriptions when pane assignments change.
