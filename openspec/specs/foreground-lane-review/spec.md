...
- **THEN** there is no syntax exemption: regex literals and slash-delimited phrases in public prose may be replaced by the absolute-path placeholder without the matcher itself refusing the record, reviewers should spell patterns in words, escaped external paths refuse on residual or protected-value inspection, an opening parenthesis after a concrete match refuses as ambiguous residue, a closing parenthesis followed by a non-whitespace character also refuses while a standalone closing delimiter publishes, paths inside command substitutions are sanitized, protected paths refuse, a spaced-path scan continues across separator-free tokens and refuses a separator-bearing suffix, and adjacent in-repository paths convert safely.
