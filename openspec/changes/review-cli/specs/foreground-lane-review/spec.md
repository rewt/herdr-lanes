...
- **WHEN** projected text contains JavaScript regex literals, slash-delimited phrases, shell command substitutions, contiguous POSIX paths, drive-letter paths, UNC paths, file URLs, or adjacent in-repository paths
- **THEN** there is no syntax exemption: regex literals and slash-delimited phrases in public prose may be replaced by the absolute-path placeholder without refusing the record, reviewers should spell patterns in words, paths inside command substitutions are sanitized, protected paths refuse, a spaced-path scan continues across separator-free tokens and refuses a separator-bearing suffix, and adjacent in-repository paths convert safely.
