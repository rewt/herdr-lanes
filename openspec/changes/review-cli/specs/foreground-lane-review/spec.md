...
- **THEN** there is no syntax exemption: regex literals and slash-delimited phrases in public prose may be replaced by the absolute-path placeholder without the matcher itself refusing the record, reviewers should spell patterns in words, escaped external paths refuse on residual or protected-value inspection, an opening parenthesis after a concrete match refuses as ambiguous residue, a closing parenthesis followed by a non-whitespace character also refuses while a standalone closing delimiter publishes, paths inside command substitutions are sanitized, protected paths refuse, closing prose delimiters before a space do not bypass the spaced-path scan, the scan continues across separator-free tokens and refuses a separator-bearing suffix, concrete matches with an unmatched separator-bearing prefix or suffix do not make a token independently sanitizable, and adjacent in-repository paths convert safely.
